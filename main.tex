...
\midrule
\intenttype{epipe} &
  SDP[0] 源=site-a, SDP[1] 源=site-b（双向性）；VLAN 一致；设备不同 \\
\intenttype{tunnel} &
  source-ne-id $\neq$ destination-ne-id \\
\intenttype{vprn} &
  所有站点 device-id 互不相同；RD 格式为 \texttt{ASN:ID} \\
\intenttype{vpls} / \intenttype{evpn-vpls} &
  站点 device-id 互不相同；外层 VLAN 标签一致 \\
\intenttype{etree} &
  $\geq$1 root + $\geq$1 leaf SAP；站点 device-id 互不相同 \\
\intenttype{cpipe} &
  设备不同；time-slots 匹配；vc-type 在合法枚举中 \\
\intenttype{evpn-epipe} &
  evpn-type 与 mpls/vxlan 子树匹配；eth-tag = access VLAN \\
\bottomrule
\end{tabular}
\end{table}
...
目标模块               & \makecell[l]{q/k/v/o\_proj (attention) +\\gate/up/down\_proj (FFN)} \\
Dropout                & 0.05 \\
...
\midrule
M1 & YANG 验证器 &
  Schema 解析器 + Tier 1+2 验证；3 种基础类型校验 &
  验证器代码 + 测试 \\
\addlinespace
M2 & 路径统一 &
  YANG 驱动路径解析替换手写映射 &
...
\addlinespace
M2.5 & 推理对齐 &
  chat template 对齐；max\_tokens 修复 &
  推理代码 + 报告 \\
\addlinespace
M3 & 横向扩展 &
  3 $\to$ 9 种意图类型；字段定义 + 模板 + Tier 4 规则 &
  完整 9 类型数据集 \\
\addlinespace
M3.5 & 质量冲刺 &
  纯函数生成器；E-Tree SDP 修复；Tier 6 &
  最终训练 + 评估 \\
\bottomrule
\end{tabular}
\end{table}
