...
Target modules           & \makecell[l]{q/k/v/o\_proj (attention) +\\gate/up/down\_proj (FFN)} \\
Dropout                  & 0.05 \\
...
\midrule
M1 & YANG Validator &
  Schema parser + Tier 1+2 validation for 3 base types &
  Validator code + tests \\
\addlinespace
M2 & Path Unification &
  YANG-driven path resolution replacing hand-written mappings &
  Equivalence regression tests \\
\addlinespace
M2.5 & Inference Align &
  Chat template alignment; max\_tokens fix &
  Inference code + report \\
\addlinespace
M3 & Expansion &
  3 $\to$ 9 intent types; field defs + templates + Tier 4 rules &
  Complete 9-type dataset \\
\addlinespace
M3.5 & Quality Sprint &
  Pure function generators; E-Tree SDP fix; Tier 6 &
  Final training + eval \\
\bottomrule
\end{tabular}
\end{table}
